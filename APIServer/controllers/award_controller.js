const sqlite3 = require("sqlite3").verbose();
const db = new sqlite3.Database("./db/database.db");
const selectDB = "awards";
const id = "award_id";

exports.findOne = async (req, res) => {
  try {
    if (!req.body.name_award)
      return res.status(400).json(`Favor informa o name_award`);

    db.all(
      `SELECT * FROM ${selectDB} WHERE name_award = (?)`,
      req.body.name_award,
      (error, rows) => {
        if (error) return res.status(502).json(`Erro na requisição`);
        if (rows.length == 0)
          return res.status(406).json(`usuário não localizado`);

        res.status(200).json(rows[0]);
      }
    );
  } catch (error) {
    res.status(500).json(`Erro interno`);
  }
};

exports.insert = async (req, res) => {
  try {
    if (!req.body.prize_award || !req.body.name_award)
      return res.status(400).json(`Body inválido`);

    db.all(
      `SELECT * FROM ${selectDB} WHERE name_award = (?)`,
      req.body.name_award,
      (error, rows) => {
        if (error) return res.status(502).json(`Erro na requisição`);
        if (rows.length == 1)
          return res.status(406).json(`Essa premiação já existente`);

        let insert = db.prepare(
          `INSERT INTO ${selectDB} (name_award, prize_award) VALUES (?, ?);`,
          req.body.name_award,
          req.body.prize_award
        );
        insert.run();
        insert.finalize();

        res.status(201).json(`Premiação cadastrado com sucesso!`);
      }
    );
  } catch (error) {
    console.log(error);
    res.status(500).json(`Erro interno`);
  }
};

exports.update = async (req, res) => {
  try {
    if (!req.params.id) return res.status(400).json(`Favor informa o ${id}`);
    if (!(req.body.name_award || req.body.prize_award))
      return res.status(400).json(`Body da requisição inválido`);

    db.all(
      `SELECT * FROM ${selectDB} WHERE ${id} = (?)`,
      req.params.id,
      (error, rows) => {
        if (error) return res.status(502).json(`Erro na requisição`);
        if (rows.length == 0)
          return res.status(406).json(`Produto não localizado`);

        if (Object.keys(req.body).includes(id))
          return res.status(403).json(`Campo ${id} Não pode ser alterado`);

        for (let key of Object.keys(req.body)) {
          let update = db.prepare(
            `UPDATE ${selectDB} SET ${key} = (?) WHERE ${id} = (?);`,
            req.body[key],
            rows[0][id]
          );
          update.run();
          update.finalize();
        }
        return res.status(201).json(`Atualizado com sucesso!`);
      }
    );
  } catch (error) {
    res.status(500).json(`Erro interno`);
  }
};

exports.delete = async (req, res) => {};
